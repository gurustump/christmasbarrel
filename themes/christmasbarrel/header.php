...
<body <?php body_class(); ?>>
	...
	<header id="header" role="header">
		<div class="header-inner-wrapper">
			<div id="main-logo">
				<?php echo is_front_page()?'<h1>':'<div class="main-logo">';?><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a><?php echo is_front_page()?'</h1>':'</div>';?>
			</div>
			<?php 
				date_default_timezone_set('Pacific/Honolulu');
				$christmas_day = mktime(0,0,0,12,25,2013);
				$today = time();
				$difference = ($christmas_day - $today);
				$days = (int) ($difference / 86400) + 1;
			?>
			<div class="calendar-drop-down">
				<a href="#" class="drop-down-activator">Days 'til  Christmas</a>
				<ul class="vid-nav">
					<?php $adventPosts = get_posts(array('category_name' => 'advent','posts_per_page'=>-1)); ?>
					<?php foreach($adventPosts as $post) { ?>
					<li>
						<?php echo customPostVideoThumbnail(get_the_ID(),'vid-thumb-small'); ?>
					</li>
					<?php } ?>
				</ul>
			</div>
			<h2 class="days-til">
				Days 'til Christmas: <span class="count"><?php echo $days; ?></span>
			</h2>
		</div>
	</header>
	<div id="wrapper">
