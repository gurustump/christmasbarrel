...
//  ____________INTERNAL METHODS____________
	...
	function page_init() {
		extract( $this->args );
		if ( ! $toplevel ) {
			$this->pagehook = add_submenu_page( VOTES_TYPE, $page_title, $menu_title, $capability, $page_slug, array( $this, '_page_content_hook' ) );
		} else {
		...
		add_submenu_page(VOTES_TYPE, __('Plugin License','voting-contest'), __('License','voting-contest'), 'publish_pages', 'votes-license', 'wp_voting_software_license_page');
	...
	private function check_args() {
		...
		$this->args = wp_parse_args( $this->args, array(
			...
			'icon_url'              => '',
			'screen_icon'           => '',
			'parent' => 'edit.php?post_type=' . VOTES_TYPE,
